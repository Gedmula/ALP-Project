    ...
    with open(path, "w", newline="") as f:
        ...
        for r in sorted(results, key=lambda x: (x["name"], x["m"])):
            ...
            w.writerow({
                ...
                "time_s":              f"{r['time']:.2f}",
                "n_iter_done":         r.get("n_iter_done", ""),
                "n_iter_budget":       r.get("n_iter_budget", ""),
...
def _save_verification_txt(results: List[dict], output_dir: Path) -> None:
    ...
    with open(path, "w") as f:
        ...
        for r in sorted(results, key=lambda x: (x["name"], x["m"])):
            ...
            # Per-operator stats across all SA chains
            ops = r.get("op_stats_total", {})
            if ops:
                tot_t = sum(d["time_s"] for d in ops.values()) or 1.0
                f.write("  Operator stats — all chains "
                        "(selected / valid / time):\n")
                for op, d in sorted(ops.items(),
                                    key=lambda kv: -kv[1]["time_s"]):
                    pct = 100.0 * d["time_s"] / tot_t
                    f.write(f"    {op:<4} sel={d['selected']:>8}  "
                            f"valid={d['valid']:>8}  "
                            f"time={d['time_s']:>9.2f}s ({pct:4.1f}%)\n")
...
def _env_versions() -> Dict[str, Any]:
    """
    Record versions of performance-critical optional dependencies.

    A run with numba (JIT feasibility checks) or without optuna (silent
    fallback to default SA params) is NOT comparable to one without/with —
    this block makes such contamination visible in run_metadata.json.
    """
    import importlib
    import platform as _platform
    out: Dict[str, Any] = {"python": _platform.python_version()}
    for mod in ("numpy", "scipy", "numba", "optuna", "torch"):
        try:
...
def _save_run_metadata_json(results: List[dict], output_dir: Path) -> None:
    ...
    payload: Dict[str, Any] = {
        ...
        "config": {
            ...
            "WALL_CLOCK_PHASING": WALL_CLOCK_PHASING,
            "WALL_CLOCK_PHASING_MAX_N": WALL_CLOCK_PHASING_MAX_N,
            "DELTA_EVAL": DELTA_EVAL,
            "DELTA_FINALISTS_K": DELTA_FINALISTS_K,
            "SA_CHAIN_DIVERSITY": SA_CHAIN_DIVERSITY,
            "SA_TABU_ENABLED": SA_TABU_ENABLED,
            "SA_TABU_TENURE": SA_TABU_TENURE,
            "SA_TABU_MODE": SA_TABU_MODE,
            "SA_TABU_ASPIRATION": SA_TABU_ASPIRATION,
            "SA_TABU_FALLBACK": SA_TABU_FALLBACK,
            "SA_TABU_REACTIVE_MIN": SA_TABU_REACTIVE_MIN,
            "SA_TABU_REACTIVE_MAX": SA_TABU_REACTIVE_MAX,
            "LP_IMPACT_INIT": LP_IMPACT_INIT,
            "BOTTLENECK_LNS": BOTTLENECK_LNS,
            "ASSIGNMENT_REPAIR": ASSIGNMENT_REPAIR,
            "SET_PARTITION_RECOMBINE": SET_PARTITION_RECOMBINE,
    ...
    for r in sorted(results, key=lambda x: (x["name"], x["m"])):
        ...
        payload["results"].append({
            ...
            "iterations": {
                "best_chain_done":   r.get("n_iter_done", 0),
                "best_chain_budget": r.get("n_iter_budget", 0),
                "chains":            r.get("chain_iters", []),
            },
            "operators": {
                "all_chains": r.get("op_stats_total", {}),
                "best_chain": r.get("op_stats_best_chain", {}),
            },
            "tabu": {
                "best_chain": r.get("tabu_stats_best_chain", {}),
                "all_chains": r.get("tabu_stats_all_chains", []),
            },
            "elite_pool_size":    r.get("elite_pool_size", 0),
            "n_elite_solutions":  len(r.get("elite_solutions", [])),
            "relinking_improved": r.get("relinking_improved", False),
            "set_partition_improved": r.get("set_partition_improved", False),
            "n_seeds_evaluated":  len(portfolio),
            "seed_portfolio": [
                {"label":   lbl,
            ...
            "sa_params": {
                ...
                "lp_repair_interval": getattr(p, "lp_repair_interval", None),
