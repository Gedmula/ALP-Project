...
$$1 | r_j, s_jk, δ̄_{j} | Σ(g_j E_j + h_j T_j)$$
...
Given a set J = {1, …, n} of aircraft, each characterised by:
...
Returns a `(SAParams, n_ils_restarts)` tuple scaled to instance size. Small instances (n ≤ 20) use tight parameters with no ILS; large instances (n > 150) use slow cooling and up to six ILS restarts per chain.
...
| MDD | Modified Due Date — minimises max($$δ_j$$, earliest feasible time) |
